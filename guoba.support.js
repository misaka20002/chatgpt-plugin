    ...
    configInfo: {
      ...
      schemas: [
        ...
        },
        {
          ...
          bottomHelpMessage: '呆毛版 对话的前面加上gemini的识图结果；1、建议关闭其他识别功能（尤其是楼上的最简单的OCR识别）；2、建议仅在所使用的ai引擎不支持图片识别时开启，例如使用gemini时开启，使用gpt或claude时关闭；3、需要配置了gemini的key才能使用；4、需要同时包含图片和消息才生效，是否生效在控制台通过输出给ai的文本判断；5、gemini遇到涩涩会中断',
          component: 'Switch'
        },
        {
        ...
        },
        {
          ...
          label: 'tts语音 切片生成',
          bottomHelpMessage: '使用切片生成而不是普通生成，可以突破字数300的限制，可以控制段间停顿和句间停顿；但1、会增加生成耗时，2、会导致每一段句子语气不一致，3、增加post失败概率。（2024年3月7日 API更新了，目前只支持切片生成，所有语音已强制使用切片生成）',
          component: 'Switch'
        },
        {
