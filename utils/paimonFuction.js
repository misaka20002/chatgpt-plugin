        ...
        if (img?.[0]) {
            ...
            try {
                let res = await client.sendMessage(msg, {
                    image: base64Image,
                    system: '你是一个专业的图像识别和分析助手。如果图片中有角色的话请指出图片中的角色的名称？来自什么作品？并需要注意以下几点：；1. 逐步、系统地扫描图像的每个区域；2. 准确识别图像中的主要对象、人物、场景；3. 描述图像的细节特征；4. 如有不确定的地方，说明可能性和置信度；5. 避免主观臆测，只描述实际看到的内容；6.请使用中文描述；请使用清晰、客观、专业的语言进行图像分析。'
                })
