...
4. 内容宽度按 1100px 设计：根容器用 width:100%（推荐，容器宽 1107px）或 width:1100px / max-width:1100px，**不要超过 1100px**。不要用 vw / vh / position:fixed / position:sticky（截图按容器高度裁剪，这些会跑出画面或留白）。表格、代码块用 width:100% + word-break:break-word，避免横向溢出。
...
- 字号层级：主标题 30px/700 · 副标题 15px #8a7671 · 区块标题 18px/700 · 正文 16px/1.7 · 注释 13px
- 外层容器：padding 28px；区块卡片：background #fff、border 1px solid 边框色、border-radius 16px、padding 18px 20px、margin-bottom 16px
- 圆角统一 10–16px，阴影只用极轻的一层（如 0 4px 15px rgba(244,190,190,0.18)）；重点信息靠字号、字重、颜色层级表达，不要堆渐变。
...
- 根元素必须带 viewBox 并自适应宽度（宽度已由容器限制在 1107px，不要再写死像素）：
...
- 重要图形距 viewBox 边缘留 24–40px 安全区，主体、箭头、文字都不要贴边，避免被截图裁到。
...
- 链路 / 流程：flex + flex-wrap + gap；节点是圆角小卡片（可带一行 13px #8a7671 注释），节点之间用 → 或 + 连接；分阶段用「① ② ③」小标题 + 左侧色标
- 分组并列：每类一个浅色底卡片（底色 / 边框 / 文字用同组辅助色），顶部 18px 加粗标签
- 对比：两列 grid（gap 16px），各自一张浅底卡片，顶部彩色标签区分
- 时间线：左侧竖线 + 圆点，每项「时间（主色加粗）+ 事件」
- 指标：3–4 个数字卡横排，数字 28px 主色粗体，说明 13px #8a7671
- 表格：圆角外框 + 浅粉表头（底 #ffe5e7、字 #d3607c）+ 行分隔线；最后一列不要再画竖线
- 引用 / 提示：左侧 5px 主色竖线 + 半透明白底 + 圆角
...
<div style="width:100%;box-sizing:border-box;padding:28px;font-family:-apple-system,'PingFang SC','Microsoft YaHei',sans-serif;color:#4a3735;">
  <div style="font-size:30px;font-weight:700;margin-bottom:6px;">AI 语音会话完整链路</div>
  <div style="font-size:15px;color:#8a7671;margin-bottom:20px;">麦克风 → VAD → STT → LLM → TTS，全程本机 + 外部 API</div>

  <div style="background:#fff;border:1px solid rgba(244,219,216,0.9);border-radius:16px;padding:18px 20px;margin-bottom:16px;">
    <div style="font-size:18px;font-weight:700;color:#d3607c;margin-bottom:12px;">① 采集与转写</div>
    <div style="display:flex;flex-wrap:wrap;align-items:center;gap:8px;">
      <div style="background:#ffe5e7;border:1px solid rgba(244,219,216,0.9);border-radius:10px;padding:10px 14px;font-size:16px;white-space:nowrap;">麦克风</div>
      <div style="color:#c8a7a4;">→</div>
      <div style="background:#eff6ff;border:1px solid #bfdbfe;border-radius:10px;padding:10px 14px;font-size:16px;">Silero VAD<br><span style="font-size:13px;color:#8a7671;">人声检测</span></div>
      <div style="color:#c8a7a4;">→</div>
      <div style="background:#eff6ff;border:1px solid #bfdbfe;border-radius:10px;padding:10px 14px;font-size:16px;">Whisper STT<br><span style="font-size:13px;color:#8a7671;">语音转文字</span></div>
    </div>
  </div>

  <div style="background:#fff;border:1px solid rgba(244,219,216,0.9);border-radius:16px;padding:18px 20px;">
    <div style="font-size:18px;font-weight:700;color:#d3607c;margin-bottom:12px;">② 推理与合成</div>
    <div style="display:flex;flex-wrap:wrap;align-items:center;gap:8px;">
      <div style="background:#f5f3ff;border:1px solid #ddd6fe;border-radius:10px;padding:10px 14px;font-size:16px;">LLM<br><span style="font-size:13px;color:#8a7671;">GLM / Kimi / DeepSeek</span></div>
      <div style="color:#c8a7a4;">→</div>
      <div style="background:#ff8fa3;color:#fff;border-radius:10px;padding:12px 16px;font-size:16px;font-weight:600;">OmniVoice TTS<br><span style="font-size:13px;font-weight:400;">音色克隆 · 生成语音</span></div>
    </div>
    <div style="font-size:13px;color:#8a7671;margin-top:12px;line-height:1.7;">引擎以「语音波形」驱动口型重演，人物身份保持不变。</div>
  </div>
</div>
</html>
