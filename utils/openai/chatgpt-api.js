...
export class ChatGPTAPI {
     ...
     */
    constructor(opts) {
        const { apiKey, apiOrg, apiBaseUrl = 'https://api.openai.com/v1', debug = false, messageStore, completionParams, systemMessage, maxModelTokens = 4000, maxResponseTokens = 8192, getMessageById, upsertMessage, fetch = globalFetch } = opts;
        this._apiKey = apiKey;
        this._apiOrg = apiOrg;
     ...
     */
    async sendMessage(text, opts = {}, role = 'user') {
        ...
        const { messages, maxTokens, numTokens } = await this._buildMessages(text, role, opts, completionParams);
    ...
    async _buildMessages(text, role, opts, completionParams) {
