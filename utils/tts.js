    ...
    : Config.ttsSpace?.includes('hf.space') ? getSpeakers("speakers_vits_uma_genshin_honkai")
        : Config.ttsSpace?.includes("hailuo") ? getSpeakers("speakers_hailuo")
            : Config.ttsSpace?.includes("ai_hobbyist") ? getSpeakers("speakers_ai_hobbyist")
                : Config.ttsSpace?.includes("Bert-VITS-Umamusume-Genshin-HonkaiSR") ? getSpeakers("bert_VITS_Umamusume_Genshin_HonkaiSR")
...
export async function generateVitsAudio(text, speaker = '随机', language = '中日混合（中文用[ZH][ZH]包裹起来，日文用[JA][JA]包裹起来）', noiseScale = parseFloat(Config.noiseScale), noiseScaleW = parseFloat(Config.noiseScaleW), lengthScale = parseFloat(Config.lengthScale)) {
    ...
    else if (space == 'https://huggingface.co/spaces/TLME/Bert-VITS-Umamusume-Genshin-HonkaiSR') {
        text = text.substr(0, 99);
        logger.info(`[chatgpt-tts]使用hf-bert-vits生成语音，角色：${speaker}，文本：\n${text}`)
        let result = {}
        let err_msg = ''
        for (let i = 0; i < 3; i++) {
            try {
                result = await connectToWss_hf({ speaker, text, noiseScale, noiseScaleW, lengthScale, wsTimeout: 180 });
                if (Config.debug)
                    logger.info(`[chatgpt-tts]共${i + 1}次尝试hf-bert-vits语音合成返回结果：${JSON.stringify(result)}`)
            } catch (error) {
                if (Config.debug)
                    logger.error(`[chatgpt-tts]共${i + 1}次尝试连接到hf-bert-vits语音合成失败：${error.message}`)
                if (i == 2)
                    err_msg = `[chatgpt-tts]共${i + 1}次尝试连接到hf-bert-vits语音合成失败`
            }
            if (result.audio_url) break;
        }
        if (!result.audio_url) throw { message: err_msg }

        return `https://tlme-bert-vits-umamusume-genshin-honkaisr.hf.space/--replicas/s96m8/file=${result.audio_url}`
    }
    ...
    //     for (let i = 0; i < 3; i++) {
    //         try {
    //             voiceUrl = await connectToWss_FISH({ speaker: speaker, text: text, config_referenceAudioPath: Config.exampleAudio, wsTimeout: 60 * (i + 1) });
    //         } catch (error) {
    //             if (Config.debug)
...
/**wss连接 hf bert vits
 * { speaker, text, noiseScale, noiseScaleW, lengthScale, wsTimeout }
 * wsTimeout: 等待时间
 */
async function connectToWss_hf(result = {}) {
    let lock = true
    let fn_index = 0
    let wss = 'wss://tlme-bert-vits-umamusume-genshin-honkaisr.hf.space/queue/join'
    function generateSessionHash() {
        return crypto.randomBytes(8).toString('hex').substring(0, 11);
    }
    let session_hash = generateSessionHash();
    let socket_trunk = {}
    let get_error
    ...
    if (true) {
        ...
        socket_1.addEventListener('message', function (event) {
            ...
            let send_data = { "data": [result.text, result.speaker, parseFloat(Config.sdp_ratio), result.noiseScale, result.noiseScaleW, result.lengthScale, Config.tts_language], "event_data": null, "fn_index": fn_index, "session_hash": session_hash }
            ...
            if (data.msg == "send_hash") {
                ...
                result = {
                    ...result,
                    audio_url: data.output.data[1].name,
                    audio_name: data.output.data[1].orig_name,
                    is_file: data.output.data[1].is_file,
...
/**wss连接 FISH */
async function connectToWss_FISH(result = {}) {
    let lock = true
    let fn_index = 1
...
async function post_to_api_fish_audio_for_taskId(text) {
    ...
    let api_fish_audio_tokenArrayUsage = accountIdArray.map(async accountId => {
        ...
        return {
            ...
            usage: (usage_accountId || 0) * 1,
            errorTimes: (errorTimes_accountId || 0) * 1
        };
    });
