...
- 工具相关：`npm run test:tools`（GithubTool 行为 + 工具鉴权上下文合并 + `test/htmlTool.check.mjs`：`generate_html` 的源码提取、可执行标签与 `<meta>`/`<link>` 清洗与落盘文件名的纯函数 + `test/render/htmlRender.template.check.mjs`：htmlRender 模板脚本的宽容/上限逻辑，无浏览器 + `test/htmlToolSend.test.mjs`：`generate_html` 的 `send_html_file` 分支——默认只发图、开启时补发与渲染同源的清洗后 `.html`、适配器无 `segment.file` 时如实回填"未发送"、只有布尔 `true` 才开启；渲染与子模型用 `mock.module` 隔离，落盘写真实文件）。
...
- **本地预渲染模板改动的做法**（改 `resources/**/index.html` 时不必启动 Yunzai 就能看出图、量尺寸）：用一次性脚本（放系统临时目录或即用即删）——
  ...
  8. **自己写预览页时的两个保真坑**（与第 1 条同源，都是 harness 伪影被当成模板 bug）：
     - 用 Python（或任何文本模式）落盘预览页必须 `newline=''`：否则 `\n` 被二次翻译成 `\r\n`、写进去的 `\r\n` 变成 `\r\r\n`，HTML 解析时**每个裸 `\r` 又变 `\n`**，页面上"每行后面多一个空行"。markdown 语义随之改变——`> a` + 空行 + `> b` 本该是一个引用块（markdown-it 实测输出 `<blockquote><p>a</p><p>b</p></blockquote>`），会被拆成"引用块 / 空引用块 / 引用块"三段，看着极像模板或 markdown-it 坏了。落盘后量一下 `#raw-markdown` 的 `textContent` 字符码自检（换行处应为 `10,10`）；
     - 从 `document.title` 读回测量数据时**只放 ASCII**：PowerShell 会按控制台输出编码（本机 GBK）解码子进程 stdout，`--dump-dom` 的 UTF-8 中文被转乱时**会吞掉紧跟其后的那个字符**（实测吃掉 JSON 的收尾引号，报 `Expecting ',' delimiter`）。需要在结果里区分对象就用下标 + 深度，不要塞中文。
  9. worktree / 独立检出里没有 `node_modules` 时也能跑这些套件：ESM **不认** `NODE_PATH`，把主检出的依赖联进来即可（`node_modules/` 本就在 `.gitignore`）——`New-Item -ItemType Junction -Path <worktree>\node_modules -Target <云崽根>\node_modules`，跑完删掉，`git status` 全程干净。`git rev-parse --git-common-dir` 可确认两个目录共享同一个 `.git`。套件输出记得 `*>` 重定向到日志文件再读（PowerShell 工具常拿不到 stdout）。
